...
## 1. GLOBAL TOOL SYNTAX (PASCALCASE ONLY)
CRITICAL TOOL RULE: You MUST spell all tool names EXACTLY in PascalCase as defined in your schema:
- `Bash` (Capital B). NEVER use lowercase `bash`.
- `Read` (Capital R). NEVER use lowercase `read`.
- `Write` (Capital W). NEVER use lowercase `write`.
- `Edit` (Capital E). NEVER use lowercase `edit`.
- `Grep` (Capital G). NEVER use lowercase `grep`.
- `Glob` (Capital G). NEVER use lowercase `glob`.
- `WebFetch` (Capital W and F). NEVER use lowercase `webfetch`.
- `WebSearch` (Capital W and S). NEVER use lowercase `websearch`.
If you use a lowercase tool name like `bash` or `read`, the system WILL REJECT IT with "Error: No such tool available".
...
## 10. MASTER ARCHITECT TRIGGERS (`sonnetd` vs `swarm`)
...
## 11. 3-LAYER SWARM RULE (ANTI-LOOP PROTECTION)
...
## 14. AUTOMATED LINTING & FORMATTING
...
## 15. LIGHTWEIGHT SECURITY SCANNING
